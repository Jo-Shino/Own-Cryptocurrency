...
contract('DappToken', function(accounts) {
  ...
  it ('transfers token ownership', function() {
    return DappToken.deployed().then(function(instance) {
      ...
      assert.equal(receipt.logs[0].event, 'Transfer', 'shold be the "Transfer" event');
  ...
  it('approve tokens for delegated transfer', function() {
    return DappToken.deployed().then(function(instance) {
      ...
      assert.equal(receipt.logs.length, 1, 'triggers one event');
      assert.equal(receipt.logs[0].event, 'Approval', 'shold be the "Approval" event');
...
　　
  it('handle delegate token transfers', function() {
    return DappToken.deployed().then(function(instance) {
      tokenInstance = instance;
